...
void Disturbance(const float state0[13], float state[13]) {
    ...
    float T[3] = {state0[10], state0[11], state0[12]};
    ...
    if (phi < 1.5709 && phi > 1.5707) {
        phi = M_PI/2;
    } else if (phi > -1.5709 && phi < -1.5707) {
        phi = -M_PI/2;
    }
...
int main() {
    ...
    for (;;) {
        ...
        if (n > 0) {
            if (ch == '\n') {
                float w1, w2, w3, phi, theta, psi, q1, q2, q3, eta, T1, T2, T3;
                std::stringstream ss(message);
                if (!(ss >> w1 >> w2 >> w3 >> phi >> theta >> psi >> q1 >> q2 >> q3 >> eta >> T1 >> T2 >> T3)) {
                    std::cout << "Parse failed on: " << message << std::endl;
                } else {
                    float state0[13] = {w1, w2, w3, phi, theta, psi, q1, q2, q3, eta, T1, T2, T3};
                    float state[10];
                    Disturbance(state0, state);
